...
private:
    ...
    UNode* search(UNode* node, string username);
    void printInOrder(UNode* node) const;
    void deleteTree(UNode* node);
    UNode* insertNode(UNode* node, Account acc);
    UNode* rightRotation(UNode* node);
    UNode* leftRotation(UNode* node);
    bool leftHeavy(UNode* node);
    bool rightHeavy(UNode* node);
    UNode* findMin(UNode* node);
    UNode* deletion(UNode* node, string username);
};
